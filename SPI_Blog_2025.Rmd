...
library(dplyr)
library(tidyr)
# Percentage point increase in the SPI
spi_change_country_avg <- df_SPI_index %>%
...
spi_change_country_avg
# e.g. use in text:
# glue::glue("SPI scores have improved worldwide by an average of {round(spi_change_country_avg, 1)}% since 2016.")
...
# 1. Compute SPI growth from 2023 to 2024
spi_growth_23_24 <- df_SPI_index %>%
  filter(date %in% c(2023, 2024)) %>%
  select(country, date, SPI.INDEX) %>%
  tidyr::pivot_wider(
    names_from  = date,
    values_from = SPI.INDEX,
  ...
  filter(!is.na(SPI_2023), !is.na(SPI_2024)) %>%
  mutate(
    growth_pp = SPI_2024 - SPI_2023,
    ...
    # Harmonise names for map_data("world")
    region = case_when(
      country == "Korea, Rep."              ~ "South Korea",
      country == "United States"            ~ "USA",
      country == "Russian Federation"       ~ "Russia",
...
# 3. Join map polygons with growth categories
world_spi_growth <- world %>%
  left_join(spi_growth_23_24, by = "region")

# 4. Plot map
ggplot(world_spi_growth, aes(long, lat, group = group)) +
  ...
  scale_fill_manual(
    values = c(
      ...
      "Decrease"               = "#D7301F"    # deep red/orange
    ),
    name = "SPI change (p.p.)\n2023 → 2024",
    drop = FALSE
  ) +
  ...
  coord_quickmap() +
  labs(
    title    = "Change in SPI Overall Score by Country, 2023–2024",
...
# Custom palette: orange (low) → teal (high)
spi_palette <- c(
  "1" = "#F98400",   # Q1 (lowest)
  "2" = "#F2C14E",
  "3" = "#ABDDA4",
...
ggplot(world_spi, aes(long, lat, group = group)) +
  geom_polygon(aes(fill = factor(SPI_quintile)), colour = "grey70", size = 0.1) +
  
  scale_fill_manual(
    values = spi_palette,
    name   = "SPI quintile (2024)",
    labels = c(
      "Q1 (lowest)",
      "Q2",
      "Q3",
    ...
    limits = c("1", "2", "3", "4", "5"),   # << EXPLICIT LEVELS, removes NA completely
    na.value = "grey90"                    # << NA countries plotted but NOT in legend
  ) +
  
  guides(
    fill = guide_legend(
      reverse = TRUE,                      # High → Low ordering
      na.translate = FALSE                 # extra insurance
    )
  ) +
  
  coord_quickmap() +
  labs(
    title   = "SPI Overall Score by Country (Quintiles)",
    caption = "Source: World Bank Statistical Performance Indicators (SPI) Index, 2024"
  ) +
  theme_void() +
  theme(
...
map_data("world") %>%
  ...
  left_join(
    df_SPI_index %>%
      filter(date %in% c(2016, 2024)) %>%
      select(country, date, SPI.INDEX.PIL2) %>%
      tidyr::pivot_wider(
        names_from  = date,
        values_from = SPI.INDEX.PIL2,
        names_prefix = "P2_"
      ) %>%
      # keep only countries with both years
      filter(!is.na(P2_2016), !is.na(P2_2024)) %>%
      mutate(
        growth_pp = P2_2024 - P2_2016,
        ...
        # --- Harmonise names to map_data("world")$region ---
        region = case_when(
          country == "Korea, Rep."              ~ "South Korea",
          country == "United States"            ~ "USA",
          country == "Russian Federation"       ~ "Russia",
        ...
        # legend order: top = biggest increase
        growth_cat = factor(
          growth_cat,
          levels = c(
            "20+ point increase",
  ...
  # same palette as before: teal for gains, orange/red for decline
  scale_fill_manual(
    values = c(
      "20+ point increase"     = "#005F73",   # dark teal
      "10–20 point increase"   = "#4EB3D3",   # medium teal
    ...
    name = "Pillar 2 (Data Services)\nSPI change 2016 → 2024",
    drop = FALSE
  ) +
  guides(fill = guide_legend(na.translate = FALSE)) +
  ...
  coord_quickmap() +
  labs(
    title    = "Change in SPI Pillar 2 (Data Services), 2016–2024",
    subtitle = "Categorised by percentage point increase in SPI.INDEX.PIL2",
    caption  = "Source: World Bank Statistical Performance Indicators (SPI)"
  ) +
  theme_void() +
  theme(
...
map_data("world") %>%
  ...
  left_join(
    df_SPI_index %>%
      filter(date %in% c(2016, 2024)) %>%
      select(country, date, SPI.INDEX.PIL4) %>%
      tidyr::pivot_wider(
        names_from  = date,
        values_from = SPI.INDEX.PIL4,
        names_prefix = "P4_"
      ) %>%
      # keep only countries with both years
      filter(!is.na(P4_2016), !is.na(P4_2024)) %>%
      mutate(
        growth_pp = P4_2024 - P4_2016,
  ...
  coord_quickmap() +
  labs(
    title    = "Change in SPI Pillar 4 (Data Sources), 2016–2024",
    subtitle = "Categorised by percentage point increase in SPI.INDEX.PIL4",
    caption  = "Source: World Bank Statistical Performance Indicators (SPI)"
  ) +
  theme_void() +
  theme(
...
map_data("world") %>%
  ...
  left_join(
    df_SPI_index %>%
      filter(date %in% c(2016, 2024)) %>%
      select(country, date, `SPI.DIM4.1.CEN.INDEX`) %>%
      tidyr::pivot_wider(
        names_from  = date,
        values_from = `SPI.DIM4.1.CEN.INDEX`,
        names_prefix = "CEN_"
      ) %>%
      # keep only countries with both years
      filter(!is.na(CEN_2016), !is.na(CEN_2024)) %>%
      mutate(
        # percentage point change
        growth_pp = CEN_2024 - CEN_2016,
  ...
  scale_fill_manual(
    values = c(
      ...
      "10–20 point increase"   = "#4EB3D3",   # medium teal
      "0–10 point increase"    = "#F2C14E",   # amber
      "Decrease"               = "#D7301F"    # red/orange
    ),
    name = "Dim 4.1 – Census (CEN)\nscore change (pp), 2016–2024",
    drop = FALSE
  ) +
  guides(fill = guide_legend(na.translate = FALSE)) +
  ...
  coord_quickmap() +
  labs(
    title    = "Change in SPI Dimension 4.1 – Census (CEN), 2016–2024",
    subtitle = "Percentage point change in SPI.DIM4.1.CEN.INDEX",
    caption  = "Source: Statistical Performance Indicators (SPI)"
  ) +
  theme_void() +
  theme(
    legend.position = "right",
...
map_data("world") %>%
  ...
  left_join(
    df_SPI_index %>%
      filter(date %in% c(2016, 2024)) %>%
      select(country, date, `SPI.DIM4.1.SVY.INDEX`) %>%
      tidyr::pivot_wider(
        names_from  = date,
        values_from = `SPI.DIM4.1.SVY.INDEX`,
        names_prefix = "SVY_"
      ) %>%
      # keep only countries with both years
      filter(!is.na(SVY_2016), !is.na(SVY_2024)) %>%
      mutate(
        # percentage point change
        growth_pp = SVY_2024 - SVY_2016,
  ...
  coord_quickmap() +
  labs(
    title    = "Change in SPI Dimension 4.1 – Surveys (SVY), 2016–2024",
    subtitle = "Percentage point change in SPI.DIM4.1.SVY.INDEX",
    caption  = "Source: Statistical Performance Indicators (SPI)"
  ) +
  theme_void() +
  theme(
...
df_summary <- df_SPI_index %>%
  # --- Keep 2016 & 2024 SPI and pivot wide ---
  filter(date %in% c(2016, 2024)) %>%
  select(iso3c, country, date, SPI.INDEX) %>%
  pivot_wider(
    names_from = date,
    values_from = SPI.INDEX,
    names_prefix = "SPI_"
  ...
  # --- Compute growth ---
  mutate(growth_pp = SPI_2024 - SPI_2016) %>%

  # --- Merge FCV + SIDS classifications ---
  left_join(
    class_data %>% mutate(
      SIDS = ifelse(
        iso3c %in% c(
          "ATG","BHS","BRB","BLZ","CPV","COM","CUB","DMA","DOM","FJI","GRD",
          "GUY","HTI","JAM","KIR","MDV","MHL","MUS","FSM","NRU","PLW","PNG",
          "KNA","LCA","VCT","WSM","STP","SYC","SLB","SUR","TLS","TON","TTO",
          "TUV","VUT"
        ),
        "Yes","No"
      )
  ...
  # --- Produce structured summaries in one tibble ---
  summarise(
    global_avg_growth      = mean(growth_pp, na.rm = TRUE),
    fcv_yes_avg_growth     = mean(growth_pp[fcv == "Yes"], na.rm = TRUE),
    fcv_no_avg_growth      = mean(growth_pp[fcv == "No"],  na.rm = TRUE),
    sids_yes_avg_growth    = mean(growth_pp[SIDS == "Yes"], na.rm = TRUE),
    sids_no_avg_growth     = mean(growth_pp[SIDS == "No"],  na.rm = TRUE)
  )


# top 10 gains in SPI scores from 2016
top10_movers <- df_SPI_index %>%
  # keep only 2016 and 2024
  filter(date %in% c(2016, 2024)) %>%
  select(iso3c, country, date, SPI.INDEX) %>%
  ...
  # compute changes
  mutate(
    change_pp  = SPI_2024 - SPI_2016,
    change_abs = abs(change_pp)
  ) %>%
  
  # drop missing
  filter(!is.na(change_pp)) %>%
  
  # pick top 10 by size of move
